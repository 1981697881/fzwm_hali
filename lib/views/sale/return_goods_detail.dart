...
import 'package:fzwm_hali/utils/handler_order.dart';
...
class ReturnGoodsDetail extends StatefulWidget {
  ...
  ReturnGoodsDetail({Key? key, @required this.FBillNo, @required this.tranType}) : super(key: key);

  @override
  _ReturnGoodsDetailState createState() => _ReturnGoodsDetailState(FBillNo, tranType);
}
...
class _ReturnGoodsDetailState extends State<ReturnGoodsDetail> {
  ...
  _ReturnGoodsDetailState(FBillNo, tranType) {
  ...
  getOrderList() async {
    ...
    if (orderDate.length > 0) {
      this.customerName = orderDate[0]['FCustName'];
      this.customerNumber = orderDate[0]['FCustNumber'];
      this.fOrgID = orderDate[0]['FSaleStyle'];
      hobby = [];
  ...
  saveOrder() async {
    ...
    if (this.hobby.length > 0) {
      ...
      Model['fcustId'] = this.customerNumber;
      ...
      this.hobby.forEach((element) {
        if (element[3]['value']['value'] != '0' &&
          ...
          if (this.isScanWork) {
            ...
            FEntityItem['fsourceBillNo'] = orderDate[hobbyIndex]['fsourceBillNo'];
            FEntityItem['fsourceEntryID'] = orderDate[hobbyIndex]['fsourceEntryID'];
            FEntityItem['fsourceTranType'] = orderDate[hobbyIndex]['fsourceTranType'];
          }else{
      ...
      String order = await SubmitEntity.saveSales(Model);
