...
class AllocationDetail extends StatefulWidget {
  var FBillNo;
  var tranType;
  AllocationDetail({Key ?key, @required this.FBillNo, @required this.tranType}) : super(key: key);
  @override
  _AllocationDetailState createState() => _AllocationDetailState(FBillNo,tranType);
}
...
//获取部门
  ...
  getMaterialList(barcodeData, code, str) async {
    ...
    userMap['pageNum'] = 1;
    userMap['pageSize'] = 20;
    String order = await CurrencyEntity.getScanInventory(userMap);
    if(!jsonDecode(order)['success']){
    ...
    if (materialDate != null) {
      ...
      if (number == 0 && this.fBillNo == "") {
        ...
        arr.add({
          ...
          "name": "FLastQty",
          "isHide": false,
          "value": {"label": materialDate["quantity"].toString(), "value": materialDate["quantity"].toString(),"remainder": "0"}
        });
        hobby.add(arr);
      }
      setState(() {
    ...
    if (this.hobby.length > 0) {
      ...
      this.hobby.forEach((element) {
        if (element[3]['value']['value'] != '0' &&
          ...
          FEntityItem['fsCStockId'] = element[4]['value']['value'];
          FEntityItem['fdCStockId'] = element[7]['value']['value'];
