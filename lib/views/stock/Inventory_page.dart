...
class InventoryPage extends StatefulWidget {
  var tranType;
  InventoryPage({Key ?key,@required this.tranType}) : super(key: key);

  @override
  _InventoryPageState createState() => _InventoryPageState(tranType);
}
...
class _InventoryPageState extends State<InventoryPage> {
  ...
  getOrderList() async {
    ...
    this.isScan = false;
    String order = await CurrencyEntity.pollingInv(userMap);
    ...
    if (orderDate.length > 0) {
      orderDate.forEach((value) {
        ...
        arr.add({
          ...
          "name": "FBillNo",
          "isHide": false,
          "value": {"label": value["FProcessID"], "value": value["FProcessID"], "id": value["FID"]}
        });
        arr.add({
          "title": "单据日期",
          "name": "FDate",
          "isHide": false,
          "value": {"label": value["Fdate"].substring(0, 10), "value": value["Fdate"].substring(0, 10)}
        });
        /*arr.add({
          "title": "物料名称",
          "name": "FMaterial",
