...
class PurchaseWarehousingDetail extends StatefulWidget {
  ...
  PurchaseWarehousingDetail({Key? key, @required this.FBillNo, @required this.tranType})
      : super(key: key);

  @override
  _PurchaseWarehousingDetailState createState() =>
      _PurchaseWarehousingDetailState(FBillNo, tranType);
}
...
class _PurchaseWarehousingDetailState extends State<PurchaseWarehousingDetail> {
  ...
  getBillNo() async {
    String res = await CurrencyEntity.getBillNo("1");
    if (jsonDecode(res)['success']) {
      setState(() {
    ...
    if (orderDate.length > 0) {
      ...
      orderDate.forEach((value) {
        ...
        arr.add({
          ...
          "name": "FStockID",
          "isHide": false,
          "value": {"label": "3#甲类原材料仓库", "value": "03.1"}
        });
        arr.add({
          "title": "批号",
          "name": "FLot",
        ...
        arr.add({
          ...
          "value": {
            "label": value["Fauxqty"],
            "value": value["Fauxqty"],
            "rateValue": value['FOverRate'] !="-1"? (value['Fauxqty'] + (value['Fauxqty'] * value['FOverRate'])):value['Fauxqty'],
            "overRate": value['FOverRate']
          } /*+value[12]*0.1*/
        });
        arr.add({
          "title": "最后扫描数量",
